...
std::shared_ptr<WiFiClient> HttpRequestComponent::get_wifi_client_() {
  ...
  if (this->secure_) {
    if (this->wifi_client_secure_ == nullptr) {
      ...
      BearSSL::X509List x509((char *)root_ca_pem);
      this->wifi_client_secure_->setTrustAnchor(&x509);
  ...
  if (this->wifi_client_ == nullptr) {
    this->wifi_client_ = std::make_shared<WiFiClientSecure>();
    if (this->secure_) {
      this->wifi_client_.setCACertBundle((char *)root_ca_pem)
    }
  }
